...
# NAICS Codes for R&D
naics_rnd = [541713, 541714, 541715]

# Add industry category
df_with_categories = filtered_df.withColumn(
    "industry_category",
    when(col("naics_code").isin(naics_rnd), "R&D").otherwise("Other Industries")
)
...
df_with_diversity = df_with_categories.withColumn(
    "is_diverse_business",
    sum([col(column) for column in diversity_columns]).cast("int") > 0
)
...
aggregated_df = df_with_diversity.groupBy("action_date_fiscal_year", "industry_category") \
    .agg(
        spark_sum(when(col("is_diverse_business") == True, 1).otherwise(0)).alias("diverse_business_count"),
        spark_sum(when(col("is_diverse_business") == False, 1).otherwise(0)).alias("non_diverse_business_count"),
    )
